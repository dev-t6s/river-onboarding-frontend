...
import { ApiDataResponse, CampaignData } from "@/types/meta-ads";
import * as bizSdk from "facebook-nodejs-business-sdk";

export async function GET(
    request: NextRequest
): Promise<NextResponse<ApiDataResponse<{
    data: CampaignData[];
    demographicData: any;
}>>> {
    try {
        ...
        const campaignData = await metaAdsService.getAdvancedData(
            account,
            timeRange,
            campaignId
        );
        ...
        return NextResponse.json({
            ...
            data: campaignData,
            summary: {
                records: campaignData.data.length,
